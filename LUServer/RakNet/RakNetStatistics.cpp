...
void RAK_DLL_EXPORT RakNet::StatisticsToString( RakNetStatistics *s, char *buffer, int verbosityLevel )
{
	...
	{
		sprintf(buffer,
			"Bytes per second sent     %64u\n"
			"Bytes per second received %64u\n"
	...
	{
		sprintf(buffer,
			"Actual bytes per second sent       %64u\n"
			"Actual bytes per second received   %64u\n"
			"Message bytes per second queued    %64u\n"
			"Total actual bytes sent            %64u\n"
			"Total actual bytes received        %64u\n"
			"Total message bytes queued         %64u\n"
			"Current packetloss                 %.0f%%\n"
			"Average packetloss                 %.0f%%\n"
			"Elapsed connection time in seconds %64u\n",
			s->valueOverLastSecond[ACTUAL_BYTES_SENT],
			s->valueOverLastSecond[ACTUAL_BYTES_RECEIVED],
		...
		if (s->BPSLimitByCongestionControl!=0)
		{
			char buff2[128];
			sprintf(buff2,
				"Send capacity                    %64u bytes per second (%.0f%%)\n",
	...
	else
	{
		sprintf(buffer,
			"Actual bytes per second sent         %64u\n"
			"Actual bytes per second received     %64u\n"
			"Message bytes per second sent        %64u\n"
			"Message bytes per second resent      %64u\n"
			"Message bytes per second queued      %64u\n"
			"Message bytes per second processed   %64u\n"
			"Message bytes per second ignored     %64u\n"
			"Total bytes sent                     %64u\n"
			"Total bytes received                 %64u\n"
			"Total message bytes sent             %64u\n"
			"Total message bytes resent           %64u\n"
			"Total message bytes queued           %64u\n"
			"Total message bytes received         %64u\n"
			"Total message bytes ignored          %64u\n"
			"Messages in send buffer, by priority %i,%i,%i,%i\n"
			"Bytes in send buffer, by priority    %i,%i,%i,%i\n"
			"Messages in resend buffer            %i\n"
			"Bytes in resend buffer               %64u\n"
			"Current packetloss                   %.0f%%\n"
			"Average packetloss                   %.0f%%\n"
			"Elapsed connection time in seconds   %64u\n",
			s->valueOverLastSecond[ACTUAL_BYTES_SENT],
